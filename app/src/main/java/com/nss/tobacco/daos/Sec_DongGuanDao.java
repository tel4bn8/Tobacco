...
/**
 * Created by lei on 2016/12/20.
 */

public class Sec_DongGuanDao {
    private DBHelper mDbHelper;
    private SQLiteDatabase mDb;
    private Context mContext;
    public Sec_DongGuanDao(Context context) {
        mContext=context;
     ...
     * 添加一条数据
     **/
    public boolean add(Sec_DongGuanEntity info) {
        ...
        long len = mDb.insert("dongguan",null,values);
    ...
    public boolean delete(String farmer) {
        String[] args = {farmer};
        //删除的条数
        int deleteNum = mDb.delete("dongguan", "farmer=?", args);
        if (deleteNum > 0) {
            return true;
        }
        return false;
     ...
     * 通过farmer条件来查询
     */
    public List<Sec_DongGuanEntity> searchByName(String farmer) {
        String sql = "SELECT * FROM dongguan WHERE farmer=" + "'" + farmer + "'";
    ...
    public int updateStateByFarmer(String state, String id) {
        ...
        int len= mDb.update("dongguan", values, "id=?", new String[]{id});
        return len;
    }
     ...
     * 通过采集状态条件查询
     */
    public List<Sec_DongGuanEntity> searchByState(String state){
        String sql = "SELECT * FROM dongguan WHERE state=" + "'" + state + "'";
        return ExecSQLForList(sql);
    }
     ...
     * 查询苗床管理表中所有数据
     */
    public List<Sec_DongGuanEntity> searchAll() {
        String sql = "SELECT * FROM dongguan";
        return ExecSQLForList(sql);
    }
    ...
    public void clearData() {
        ExecSQL("DELETE FROM dongguan");
    }

    /**
     * 执行SQl语句返回苗床追肥集合信息
     */
    public List<Sec_DongGuanEntity> ExecSQLForList(String sql) {
        List<Sec_DongGuanEntity> list = new ArrayList<>();
    ...
    public void updateByFarmer(String raw, String value, String farmer) {
        String sql = "UPDATE dongguan SET " + raw + "=" + "'" + value + "'" + "WHERE farmer=" + "'" + farmer + "'";
        ExecSQL(sql);
    }
    ...
    public void updateRaw(String raw, String rawValue) {
        String sql = "UPDATE dongguan SET " + raw + " = " + "'" + rawValue + "'";
        ExecSQL(sql);
    }
