...
public class Sec_XueFeiDao {
    private DBHelper mDbHelper;
    private SQLiteDatabase mDb;
    private Context mContext;
    public Sec_XueFeiDao(Context context) {
        mContext=context;
     ...
     * 添加一条数据
     **/
    public boolean add(Sec_XueFeiEntity info) {
        ...
        long len = mDb.insert("xuefei",null,values);
        if (len > 0) {
            return true;
        }
        return false;
    ...
    public boolean delete(String farmer) {
        String[] args = {farmer};
        //删除的条数
        int deleteNum = mDb.delete("xuefei", "farmer=?", args);
        if (deleteNum > 0) {
            return true;
        }
        return false;
     ...
     * 通过farmer条件来查询
     */
    public List<Sec_XueFeiEntity> searchByName(String farmer) {
        String sql = "SELECT * FROM xuefei WHERE farmer=" + "'" + farmer + "'";
    ...
    public int updateStateById(String state, String id) {
        ...
        int len= mDb.update("xuefei", values, "id=?", new String[]{id});
        return len;
    }
    /**
     * 通过采集状态条件查询
     */
    public List<Sec_XueFeiEntity> searchByState(String state){
        String sql = "SELECT * FROM xuefei WHERE state=" + "'" + state + "'";
        return ExecSQLForList(sql);
    }
     ...
     * 查询苗床管理表中所有数据
     */
    public List<Sec_XueFeiEntity> searchAll() {
        String sql = "SELECT * FROM xuefei";
        return ExecSQLForList(sql);
    }
    /**
     * 通过id来修改状态值
    ...
    public void clearData() {
        ExecSQL("DELETE FROM xuefei");
    }

    /**
     * 执行SQl语句返回基肥集合信息
     */
    public List<Sec_XueFeiEntity> ExecSQLForList(String sql) {
        List<Sec_XueFeiEntity> list = new ArrayList<>();
    ...
    public void updateByFarmer(String raw, String value, String farmer) {
        String sql = "UPDATE xuefei SET " + raw + "=" + "'" + value + "'" + "WHERE farmer=" + "'" + farmer + "'";
        ExecSQL(sql);
    }
    ...
    public void updateRaw(String raw, String rawValue) {
        String sql = "UPDATE xuefei SET " + raw + " = " + "'" + rawValue + "'";
        ExecSQL(sql);
    }
