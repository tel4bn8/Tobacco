...
public class Sec_ZhuifeiDao {
     ...
     * 添加一条数据
     **/
    public boolean add(Sec_ZhuifeiEntity info) {
        ...
        long len = mDb.insert("zhuifei",null,values);
        if (len > 0) {
            return true;
        }
        return false;
    ...
    public boolean delete(String farmer) {
        String[] args = {farmer};
        //删除的条数
        int deleteNum = mDb.delete("zhuifei", "farmer=?", args);
        if (deleteNum > 0) {
            return true;
        }
        return false;
     ...
     * 通过farmer条件来查询
     */
    public List<Sec_ZhuifeiEntity> searchByName(String farmer) {
        String sql = "SELECT * FROM zhuifei WHERE farmer=" + "'" + farmer + "'";
    ...
    public int updateStateByFarmer(String state, String id) {
        ...
        int len= mDb.update("zhuifei", values, "id=?", new String[]{id});
        return len;
    }
     ...
     * 通过采集状态条件查询
     */
    public List<Sec_ZhuifeiEntity> searchByState(String state){
        String sql = "SELECT * FROM zhuifei WHERE state=" + "'" + state + "'";
        return ExecSQLForList(sql);
    }
     ...
     * 查询追肥管理表中所有数据
     */
    public List<Sec_ZhuifeiEntity> searchAll() {
        String sql = "SELECT * FROM zhuifei";
        return ExecSQLForList(sql);
    }
    ...
    public void clearData() {
        ExecSQL("DELETE FROM zhuifei");
    }

    /**
     * 执行SQl语句返回基肥集合信息
     */
    public List<Sec_ZhuifeiEntity> ExecSQLForList(String sql) {
        List<Sec_ZhuifeiEntity> list = new ArrayList<>();
    ...
    public void updateByFarmer(String raw, String value, String farmer) {
        String sql = "UPDATE zhuifei SET " + raw + "=" + "'" + value + "'" + "WHERE farmer=" + "'" + farmer + "'";
        ExecSQL(sql);
    }
    ...
    public void updateRaw(String raw, String rawValue) {
        String sql = "UPDATE zhuifei SET " + raw + " = " + "'" + rawValue + "'";
        ExecSQL(sql);
    }
