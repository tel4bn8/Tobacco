...
public class Sec_XiaoduInfoDao {
    ...
    public Sec_XiaoduInfoDao(Context context) {
        mContext=context;
     ...
     * 添加一条数据
     **/
    public boolean add(Sec_XiaoduInfoEntity info) {
        ContentValues values = new ContentValues();
        values.put("id", info.getId());
        values.put("farmer",info.getFarmer());
        values.put("yaopin",info.getYaopin());
        values.put("sheshi",info.getSheshi());
        ...
        long len = mDb.insert("xiaodu",null,values);
        if (len > 0) {
            return true;
        }
        return false;
    ...
    public boolean delete(String farmer) {
        String[] args = {farmer};
        //删除的条数
        int deleteNum = mDb.delete("xiaodu", "farmer=?", args);
        if (deleteNum > 0) {
            return true;
        }
        return false;
     ...
     * 通过farmer条件来查询
     */
    public List<Sec_XiaoduInfoEntity> searchByName(String farmer) {
    ...
    public List<Sec_XiaoduInfoEntity> searchByState(String state){
        String sql = "SELECT * FROM xiaodu WHERE state=" + "'" + state + "'";
        return ExecSQLForList(sql);
    }
     ...
     * 查询消毒表中所有数据
     */
    public List<Sec_XiaoduInfoEntity> searchAll() {
     ...
     * 执行SQl语句返回消毒集合信息
     */
    public List<Sec_XiaoduInfoEntity> ExecSQLForList(String sql) {
        List<Sec_XiaoduInfoEntity> list = new ArrayList<>();
        ...
        while (cursor.moveToNext()) {
            ...
            info.setSheshi(cursor.getString(cursor.getColumnIndex("sheshi")));
    ...
    public int updateStateById(String state, String id) {
        ...
        int len= mDb.update("xiaodu", values, "id=?", new String[]{id});
        return len;
    }
    /**
     * 通过名字来修改值
    ...
    public void updateByFarmer(String raw, String value, String farmer) {
        String sql = "UPDATE xiaodu SET " + raw + "=" + "'" + value + "'" + "WHERE farmer=" + "'" + farmer + "'";
        ExecSQL(sql);
    }
