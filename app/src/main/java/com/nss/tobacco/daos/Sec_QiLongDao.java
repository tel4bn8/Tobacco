...
public class Sec_QiLongDao {
    private DBHelper mDbHelper;
    private SQLiteDatabase mDb;
    private Context mContext;
    public Sec_QiLongDao(Context context) {
        mContext=context;
     ...
     * 添加一条数据
     **/
    public boolean add(Sec_QiLongEntity info) {
        ...
        long len = mDb.insert("qilong",null,values);
        if (len > 0) {
            return true;
        }
        return false;
    ...
    public int updateStateByFarmer(String state, String id) {
        ...
        int len= mDb.update("qilong", values, "id=?", new String[]{id});
        return len;
    }
     ...
     * 通过采集状态条件查询
     */
    public List<Sec_QiLongEntity> searchByState(String state){
        String sql = "SELECT * FROM qilong WHERE state=" + "'" + state + "'";
    ...
    public boolean delete(String farmer) {
        String[] args = {farmer};
        //删除的条数
        int deleteNum = mDb.delete("qilong", "farmer=?", args);
        if (deleteNum > 0) {
            return true;
        }
        return false;
     ...
     * 通过farmer条件来查询
     */
    public List<Sec_QiLongEntity> searchByName(String farmer) {
        String sql = "SELECT * FROM qilong WHERE farmer=" + "'" + farmer + "'";
        return ExecSQLForList(sql);
    }
     ...
     * 查询苗床管理表中所有数据
     */
    public List<Sec_QiLongEntity> searchAll() {
        String sql = "SELECT * FROM qilong";
        return ExecSQLForList(sql);
    }
    ...
    public void clearData() {
        ExecSQL("DELETE FROM qilong");
    }

    /**
     * 执行SQl语句返回苗床追肥集合信息
     */
    public List<Sec_QiLongEntity> ExecSQLForList(String sql) {
    ...
    public void updateByFarmer(String raw, String value, String farmer) {
        String sql = "UPDATE qilong SET " + raw + "=" + "'" + value + "'" + "WHERE farmer=" + "'" + farmer + "'";
        ExecSQL(sql);
    }
    ...
    public void updateRaw(String raw, String rawValue) {
        String sql = "UPDATE qilong SET " + raw + " = " + "'" + rawValue + "'";
        ExecSQL(sql);
    }
