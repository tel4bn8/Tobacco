...
public class YnZhongzhiSpActivity extends AppCompatActivity implements View.OnClickListener {
    private ZhongzhiShenpiDao zhongzhiShenpiDao = null;

    private List<ZhongzhiShenpi> allentity; //所有数据
    private List<ZhongzhiShenpi> searchentity;//查询结果数据
    ...
    private String photoFoloder = CommonUtil.getSDPath() + File.separator + "zhongzhishenpi";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_yn_zhongzhi_sp);

        zhongzhiShenpiDao = new ZhongzhiShenpiDao(this);
        allentity = zhongzhiShenpiDao.searchAllData();
    ...
    private void initData(List<ZhongzhiShenpi> entites) {
        recyclerView.setAdapter(new CommonAdapter<ZhongzhiShenpi>(this, R.layout.item_detailmsg_recyclerview, entites) {
            @Override
            public void convert(ViewHolder holder, ZhongzhiShenpi mZhongzhiShenpi) {
                holder.setText(R.id.id_num, mZhongzhiShenpi.getCreatetime());
                holder.setText(R.id.id_farmer, mZhongzhiShenpi.getFarmer());

                if ("0".equals(mZhongzhiShenpi.getState())) {
            ...
            public void onBindViewHolder(ViewHolder holder, final int position) {
                ...
                holder.setOnClickListener(R.id.id_ll_item, new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        intent.setClass(YnZhongzhiSpActivity.this, Msg_YnZhongzhiActivity.class);

                        if (!isSearch) {
                            intent.putExtra("zhongzhishenpiInfo", allentity.get(position));
                        } else {
                            intent.putExtra("zhongzhishenpiInfo", searchentity.get(position));
                        }
    ...
    private void initView() {
        ...
        recyclerView = (RecyclerView) findViewById(R.id.frm_first_yn_zhongzhi_sp_recyclerView);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));

        mSwipeRefreshLayout = (SwipeRefreshLayout) findViewById(R.id.frm_first_yn_zhongzhi_sp_swipe);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.back_item:
                ...
                break;
            case R.id.add_item:
                Intent intent = new Intent(YnZhongzhiSpActivity.this, YnZhongzhiSpAddActivity.class);
                startActivity(intent);
                break;
            case R.id.up_item:
        ...
        if (!isSearch) {
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    allentity = zhongzhiShenpiDao.searchAllData();
                    initData(allentity);
                    Toast.makeText(YnZhongzhiSpActivity.this, "上传成功", Toast.LENGTH_SHORT).show();
                }
            }, 1000);
        }
        //搜索界面的数据
    ...
    private void upData(final List<ZhongzhiShenpi> entites) {
        for (int i = 0; i < entites.size(); i++) {
            if ("0".equals(entites.get(i).getState())) {
                ...
                final int finalI = i;
                XUtil.Post(API_Data.ZhongzhishenpiUP, map, new MyCallBack<String>() {
                    @Override
                    public void onSuccess(String result) {
                        super.onSuccess(result);
                        if (!TextUtils.isEmpty(result)) {
                            //修改该条目数据库中的状态
                            int len = zhongzhiShenpiDao.updateStateByFarmer("1", entites.get(finalI).getId());
                            if (len > 0) {
    ...
    private void searchInfoByName() {
        ...
        isSearch = true;
        searchentity = zhongzhiShenpiDao.searchData(farmer);
