...
public class WgTurangActivity extends AppCompatActivity implements View.OnClickListener{
    private TurangJianceDao turangJianceDao = null;

    private List<TurangJiance> allentity; //所有数据
    private List<TurangJiance> searchentity;//查询结果数据
    ...
    private String photoFoloder = CommonUtil.getSDPath() + File.separator + "turangjiance";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_wg_turang);

        turangJianceDao = new TurangJianceDao(this);
        allentity = turangJianceDao.searchAllData();
    ...
    //初始化数据
    private void initData(List<TurangJiance> entites) {

        recyclerView.setAdapter(new CommonAdapter<TurangJiance>(this, R.layout.item_detailmsg_recyclerview,entites) {
            @Override
            public void convert(ViewHolder holder, TurangJiance mTurangJiance) {
                holder.setText(R.id.id_num,mTurangJiance.getPicktime());
                holder.setText(R.id.id_farmer,mTurangJiance.getWanggecode());

                if("0".equals(mTurangJiance.getState())){
            ...
            public void onBindViewHolder(ViewHolder holder, final int position) {
                ...
                holder.setOnClickListener(R.id.id_ll_item,new View.OnClickListener() {
                    ...
                    public void onClick(View v){
                        ...
                        intent.setClass(WgTurangActivity.this, Msg_WgTurangActivity.class);

                        if(!isSearch){
                            intent.putExtra("turangjianceInfo", allentity.get(position));
                        }else{
                            intent.putExtra("turangjianceInfo", searchentity.get(position));
                        }
    ...
    private void initView() {
        ...
        editTextsearch.setHint("请输入您要查询的年度");
        searchButton = (Button) findViewById(R.id.search_button_item);

        recyclerView = (RecyclerView) findViewById(R.id.frm_first_wg_turang_recyclerView);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));

        mSwipeRefreshLayout = (SwipeRefreshLayout) findViewById(R.id.frm_first_wg_turang_swipe);
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.back_item:
                ...
                break;
            case R.id.add_item:
                Intent intent = new Intent(WgTurangActivity.this, WgTurangAddActivity.class);
                startActivity(intent);
                break;
        ...
        if(!isSearch){
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    allentity=turangJianceDao.searchAllData();
                    initData(allentity);
                    Toast.makeText(WgTurangActivity.this, "上传成功", Toast.LENGTH_SHORT).show();
                }
            }, 1000);
        }
        //搜索界面的数据
    ...
    //上传未上传的数据
    private void upData(final List<TurangJiance> entites) {
        ...
        for(int i=0;i<entites.size();i++){
            if("0".equals(entites.get(i).getState())){
                ...
                map.put("youjizhi",entites.get(i).getYoujizhi());
                map.put("ph",entites.get(i).getPh());
                map.put("n",entites.get(i).getN());
                map.put("p",entites.get(i).getP());
                ...
                final int finalI = i;
                XUtil.Post(API_Data.TurangjianceUP,map, new MyCallBack<String>() {
                    @Override
                    public void onSuccess(String result) {
                        super.onSuccess(result);
                        if(!TextUtils.isEmpty(result)){
                            //修改该条目数据库中的状态
                            int len = turangJianceDao.updateStateByYear("1", entites.get(finalI).getId());
                            if(len>0){
    ...
    private void searchInfoByName() {
        ...
        isSearch = true;
        searchentity = turangJianceDao.searchData(year);
