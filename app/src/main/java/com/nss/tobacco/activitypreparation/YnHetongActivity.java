...
import com.nss.tobacco.daos.HetongGuanliDao;
...
public class YnHetongActivity extends AppCompatActivity implements View.OnClickListener{
    private HetongGuanliDao hetongGuanliDao = null;

    private List<HetongGuanli> allentity; //所有数据
    private List<HetongGuanli> searchentity;//查询结果数据
    ...
    private String photoFoloder = CommonUtil.getSDPath() + File.separator + "hetongguanli";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_yn_hetong);

        hetongGuanliDao = new HetongGuanliDao(this);
        allentity = hetongGuanliDao.searchAllData();
    ...
    private void initData(List<HetongGuanli> entites) {
        recyclerView.setAdapter(new CommonAdapter<HetongGuanli>(this, R.layout.item_detailmsg_recyclerview,entites) {
            @Override
            public void convert(ViewHolder holder, HetongGuanli mHetongGuanli) {
                holder.setText(R.id.id_num,mHetongGuanli.getCaoqiantime());
                holder.setText(R.id.id_farmer,mHetongGuanli.getFarmer());

                if("0".equals(mHetongGuanli.getState())){
            ...
            public void onBindViewHolder(ViewHolder holder, final int position) {
                ...
                holder.setOnClickListener(R.id.id_ll_item,new View.OnClickListener() {
                    ...
                    public void onClick(View v){
                        ...
                        intent.setClass(YnHetongActivity.this, Msg_YnHetongActivity.class);

                        if(!isSearch){
                            intent.putExtra("hetongguanliInfo", allentity.get(position));
                        }else{
                            intent.putExtra("hetongguanliInfo", searchentity.get(position));
                        }
    ...
    private void initView() {
        ...
        recyclerView = (RecyclerView) findViewById(R.id.frm_first_yn_htgl_recyclerView);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));

        mSwipeRefreshLayout = (SwipeRefreshLayout) findViewById(R.id.frm_first_yn_htgl_swipe);
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.back_item:
                ...
                break;
            case R.id.add_item:
                Intent intent = new Intent(YnHetongActivity.this, YnHetongAddActivity.class);
                startActivity(intent);
                break;
            case R.id.up_item:
        ...
        if(!isSearch){
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    allentity=hetongGuanliDao.searchAllData();
                    initData(allentity);
                    Toast.makeText(YnHetongActivity.this, "上传成功", Toast.LENGTH_SHORT).show();
                }
            }, 1000);
        }
        //搜索界面的数据
    ...
    private void upData(final List<HetongGuanli> entites) {
        for(int i=0;i<entites.size();i++){
            if("0".equals(entites.get(i).getState())){
                ...
                map.put("code",entites.get(i).getCode());
                ...
                map.put("num1",entites.get(i).getNum1());
                map.put("caoqiantime",entites.get(i).getCaoqiantime());
                map.put("detail",entites.get(i).getDetail());
                final int finalI = i;
                XUtil.Post(API_Data.HetongUP,map, new MyCallBack<String>() {
                    @Override
                    public void onSuccess(String result) {
                        super.onSuccess(result);
                        if(!TextUtils.isEmpty(result)){
                            //修改该条目数据库中的状态
                            int len = hetongGuanliDao.updateStateByFarmer("1", entites.get(finalI).getId());
                            if(len>0){
    ...
    private void searchInfoByName() {
        ...
        isSearch=true;
        searchentity = hetongGuanliDao.searchData(farmer);
