...
public class YmhYumaioActivity extends AppCompatActivity implements View.OnClickListener{
    private YumiaohuDao yumiaohuDao = null;

    private List<Yumiaohu> allentity; //所有数据
    private List<Yumiaohu> searchentity;//查询结果数据
    ...
    private String photoFoloder = CommonUtil.getSDPath() + File.separator + "yumiaohu";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_ymh_yumaio);

        yumiaohuDao = new YumiaohuDao(this);
        allentity = yumiaohuDao.searchAllData();
    ...
    private void initData(List<Yumiaohu> entites) {
        recyclerView.setAdapter(new CommonAdapter<Yumiaohu>(this, R.layout.item_detailmsg_recyclerview,entites) {
            @Override
            public void convert(ViewHolder holder, Yumiaohu mYumiaohu) {
                holder.setText(R.id.id_num,mYumiaohu.getCreatetime());
                holder.setText(R.id.id_farmer,mYumiaohu.getName());

                if("0".equals(mYumiaohu.getState())){
            ...
            public void onBindViewHolder(ViewHolder holder, final int position) {
                ...
                holder.setOnClickListener(R.id.id_ll_item,new View.OnClickListener() {
                    ...
                    public void onClick(View v){
                        ...
                        intent.setClass(YmhYumaioActivity.this, Msg_YmhYumaioActivity.class);

                        if(!isSearch){
                            intent.putExtra("yumiaohuInfo", allentity.get(position));
                        }else{
                            intent.putExtra("yumiaohuInfo", searchentity.get(position));
                        }
    ...
    private void initView() {
        ...
        recyclerView = (RecyclerView) findViewById(R.id.frm_first_ymh_yumiaohu_recyclerView);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));

        mSwipeRefreshLayout = (SwipeRefreshLayout) findViewById(R.id.frm_first_ymh_yumiaohu_swipe);
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.back_item:
                ...
                break;
            case R.id.add_item:
                Intent intent = new Intent(YmhYumaioActivity.this, YmhYumaioAddActivity.class);
                startActivity(intent);
                break;
            case R.id.up_item:
        ...
        if(!isSearch){
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    allentity=yumiaohuDao.searchAllData();
                    initData(allentity);
                    Toast.makeText(YmhYumaioActivity.this, "上传成功", Toast.LENGTH_SHORT).show();
                }
            }, 1000);
        }
        //搜索界面的数据
    ...
    private void upData(final List<Yumiaohu> entites) {
        for(int i=0;i<entites.size();i++){
            if("0".equals(entites.get(i).getState())){
                ...
                map.put("yumiaogongchangid",entites.get(i).getYumiaogongchangid());
                map.put("detail",entites.get(i).getDetail());
                final int finalI = i;
                XUtil.Post(API_Data.YumiaohuUP,map, new MyCallBack<String>() {
                    @Override
                    public void onSuccess(String result) {
                        super.onSuccess(result);
                        if(!TextUtils.isEmpty(result)){
                            //修改该条目数据库中的状态
                            int len = yumiaohuDao.updateStateByFarmer("1", entites.get(finalI).getId());
                            if(len>0){
    ...
    private void searchInfoByName() {
        ...
        isSearch=true;
        searchentity = yumiaohuDao.searchData(name);
