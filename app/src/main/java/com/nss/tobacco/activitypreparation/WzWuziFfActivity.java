...
public class WzWuziFfActivity extends AppCompatActivity implements View.OnClickListener{
    private WuziFafangDao wuziFafangDao = null;

    private List<WuziFafang> allentity; //所有数据
    private List<WuziFafang> searchentity;//查询结果数据
    ...
    private String photoFoloder = CommonUtil.getSDPath() + File.separator + "wuzifafang";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_wz_wuzi_ff);

        wuziFafangDao = new WuziFafangDao(this);
        allentity = wuziFafangDao.searchAllData();
    ...
    private void initData(List<WuziFafang> entites) {
        recyclerView.setAdapter(new CommonAdapter<WuziFafang>(this, R.layout.item_detailmsg_recyclerview,entites) {
            @Override
            public void convert(ViewHolder holder, WuziFafang mWuziFafang) {
                holder.setText(R.id.id_num,mWuziFafang.getCreatetime());
                holder.setText(R.id.id_farmer,mWuziFafang.getFarmer());

                if("0".equals(mWuziFafang.getState())){
            ...
            public void onBindViewHolder(ViewHolder holder, final int position) {
                ...
                holder.setOnClickListener(R.id.id_ll_item,new View.OnClickListener() {
                    ...
                    public void onClick(View v){
                        ...
                        intent.setClass(WzWuziFfActivity.this, Msg_WzWuzifafangActivity.class);

                        if(!isSearch){
                            intent.putExtra("wuzifafangInfo", allentity.get(position));
                        }else{
                            intent.putExtra("wuzifafangInfo", searchentity.get(position));
                        }
    ...
    private void initView() {
        ...
        recyclerView = (RecyclerView) findViewById(R.id.frm_first_wz_wuzi_ff_recyclerView);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));

        mSwipeRefreshLayout = (SwipeRefreshLayout) findViewById(R.id.frm_first_wz_wuzi_ff_swipe);
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.back_item:
                ...
                break;
            case R.id.add_item:
                Intent intent = new Intent(WzWuziFfActivity.this, WzWuziFfAddActivity.class);
                startActivity(intent);
                break;
            case R.id.up_item:
        ...
        if(!isSearch){
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    allentity=wuziFafangDao.searchAllData();
                    initData(allentity);
                    Toast.makeText(WzWuziFfActivity.this, "上传成功", Toast.LENGTH_SHORT).show();
                }
            }, 1000);
        }
        //搜索界面的数据
    ...
    private void upData(final List<WuziFafang> entites) {
        for(int i=0;i<entites.size();i++){
            if("0".equals(entites.get(i).getState())){
                ...
                map.put("danwei",entites.get(i).getDanwei());
                map.put("money",entites.get(i).getMoney());
                map.put("createtime",entites.get(i).getCreatetime());
                map.put("detail",entites.get(i).getDetail());
                final int finalI = i;
                XUtil.Post(API_Data.WuzifafangUP,map, new MyCallBack<String>() {
                    @Override
                    public void onSuccess(String result) {
                        super.onSuccess(result);
                        if(!TextUtils.isEmpty(result)){
                            //修改该条目数据库中的状态
                            int len = wuziFafangDao.updateStateByFarmer("1", entites.get(finalI).getId());
                            if(len>0){
    ...
    private void searchInfoByName() {
        ...
        isSearch=true;
        searchentity = wuziFafangDao.searchData(farmer);
