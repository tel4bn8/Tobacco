...
public class YnYannongDaActivity extends AppCompatActivity implements View.OnClickListener {
    private YannongDao yannongDao = null;

    private List<Yannong> allentity; //所有数据
    private List<Yannong> searchentity;//查询结果数据
    ...
    private String photoFoloder = CommonUtil.getSDPath() + File.separator + "yannongdangan";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_yn_yannong_da);
        yannongDao = new YannongDao(this);
        allentity = yannongDao.searchAllData();
    ...
    private void initData(List<Yannong> entites) {
        recyclerView.setAdapter(new CommonAdapter<Yannong>(this, R.layout.item_detailmsg_recyclerview,entites) {
            @Override
            public void convert(ViewHolder holder, Yannong mYannong) {
                holder.setText(R.id.id_num,mYannong.getCreatetime());
                holder.setText(R.id.id_farmer,mYannong.getName());

                if("0".equals(mYannong.getState())){
            ...
            public void onBindViewHolder(ViewHolder holder, final int position) {
                ...
                holder.setOnClickListener(R.id.id_ll_item,new View.OnClickListener() {
                    ...
                    public void onClick(View v){
                        ...
                        intent.setClass(YnYannongDaActivity.this, Msg_WgYannongdaActivity.class);

                        if(!isSearch){
                            intent.putExtra("yannongdanganInfo", allentity.get(position));
                        }else{
                            intent.putExtra("yannongdanganInfo", searchentity.get(position));
                        }
    ...
    private void initView() {
        ...
        recyclerView = (RecyclerView) findViewById(R.id.frm_first_yn_ynda_recyclerView);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));

        mSwipeRefreshLayout = (SwipeRefreshLayout) findViewById(R.id.frm_first_yn_ynda_swipe);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.back_item:
                ...
                break;
            case R.id.add_item:
                Intent intent = new Intent(YnYannongDaActivity.this, YnYannongDaAddActivity.class);
                startActivity(intent);
                break;
        ...
        if(!isSearch){
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    allentity=yannongDao.searchAllData();
                    initData(allentity);
                    Toast.makeText(YnYannongDaActivity.this, "上传成功", Toast.LENGTH_SHORT).show();
                }
            }, 1000);
        }
        //搜索界面的数据
    ...
    private void upData(final List<Yannong> entites) {
        for(int i=0;i<entites.size();i++){
            if("0".equals(entites.get(i).getState())){
                ...
                map.put("idcard",entites.get(i).getIdcard());
                map.put("phone",entites.get(i).getPhone());
                map.put("sex",entites.get(i).getSex());
                map.put("age",entites.get(i).getAge());
                map.put("workyear",entites.get(i).getWorkyear());
                map.put("education",entites.get(i).getEducation());
                map.put("area",entites.get(i).getArea());
                map.put("year",entites.get(i).getYear());
                map.put("skill",entites.get(i).getSkill());
                map.put("credit",entites.get(i).getCredit());
                map.put("grade",entites.get(i).getGrade());
                ...
                final int finalI = i;
                XUtil.Post(API_Data.YannongdanganUP,map, new MyCallBack<String>() {
                    @Override
                    public void onSuccess(String result) {
                        super.onSuccess(result);
                        if(!TextUtils.isEmpty(result)){
                            //修改该条目数据库中的状态
                            int len = yannongDao.updateStateByName("1", entites.get(finalI).getId());
                            if(len>0){
    ...
    private void searchInfoByName() {
        ...
        isSearch=true;
        searchentity = yannongDao.searchData(name);
