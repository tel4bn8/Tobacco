...
public class WgYanyeLhAddActivity extends AppCompatActivity implements View.OnClickListener {
    private YanyeLihuaDao yanyeLihuaDao;
    //拍照
    Context context;
    private static final int CRAEMA_REQUEST_CODE = 3; //拍照请求码
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        x.view().inject(this);
        context = this;
        yanyeLihuaDao = new YanyeLihuaDao(context);
    ...
    private void initCarema() {
        ...
        if (getIntent().getStringExtra("folderName") == null) {
            photoFolderAddress = CommonUtil.getSDPath() + File.separator + "yanyelihua";
        } else {
    ...
    private void saveInfo() {
        ...
        ArrayList<YanyeLihua> yanyelihuaList = new ArrayList<>();
        YanyeLihua mYanyeLihua = new YanyeLihua();
        mYanyeLihua.setId(IdUtil.getId());
        mYanyeLihua.setWanggecode(tvWanggeID.getText().toString());
        mYanyeLihua.setYear(spYewuYear.getSelectedItem().toString());
        mYanyeLihua.setK(etK.getText().toString());
        mYanyeLihua.setCl(etCl.getText().toString());
        if(TextUtils.isEmpty(etYanjian.getText().toString())){
            Toast.makeText(this, "数据不能留空", Toast.LENGTH_SHORT).show();
            return;
        }
        mYanyeLihua.setYanjian(etYanjian.getText().toString());
        mYanyeLihua.setHuanyuantang(etHuanyuantang.getText().toString());
        mYanyeLihua.setZongtang(etZongtang.getText().toString());
        mYanyeLihua.setPicktime(tvDengjiTime.getText().toString());
        mYanyeLihua.setTechnician(tvYanjiyuan.getText().toString());
        mYanyeLihua.setDetail(etBeizhu.getText().toString());
        mYanyeLihua.setState(state);
        mYanyeLihua.setPhotopath(photopath.toString());

        yanyelihuaList.add(mYanyeLihua);
        yanyeLihuaDao.add(mYanyeLihua);
    }
