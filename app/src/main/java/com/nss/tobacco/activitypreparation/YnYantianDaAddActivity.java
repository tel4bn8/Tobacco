...
public class YnYantianDaAddActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    private static final int CRAEMA_REQUEST_CODE = 5; //拍照请求码
    ...
    @ViewInject(R.id.frm_first_yn_ytda_add_caremaView)
    ...
    private void initCarema() {
        ...
        if(getIntent().getStringExtra("folderName") == null){
            photoFolderAddress = CommonUtil.getSDPath() + File.separator + "yantiandangan";
        }else{
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.back_item:
                finish();
                break;
            case R.id.frm_first_yn_ytda_add:
                //验证sd卡是否可用
    ...
    private void saveInfo() {
        ...
        ArrayList<YantianDangan> yantiandanganList = new ArrayList<>();
        YantianDangan mYantianDangan = new YantianDangan();

        mYantianDangan.setId(IdUtil.getId());
        mYantianDangan.setFarmer(spYannongName.getSelectedItem().toString());
        mYantianDangan.setYear(spShiyongniandu.getSelectedItem().toString());
        mYantianDangan.setArea(etMianji.getText().toString());
        mYantianDangan.setLng(tvJingdu.getText().toString());
        mYantianDangan.setLat(tvWeidu.getText().toString());
        mYantianDangan.setSrc(spSoilsrc.getSelectedItem().toString());
        mYantianDangan.setDixing(spDixing.getSelectedItem().toString());
        mYantianDangan.setSoil(spSoiltype.getSelectedItem().toString());
        mYantianDangan.setPrecrop(spPrecrop.getSelectedItem().toString());
        mYantianDangan.setWater(spWater.getSelectedItem().toString());
        mYantianDangan.setDilishuiping(spDilishuiping.getSelectedItem().toString());
        mYantianDangan.setCreatetime(tvDengjiTime.getText().toString());
        mYantianDangan.setDetail(etBeizhu.getText().toString());
        mYantianDangan.setState(state);
        mYantianDangan.setPhotopath(photoPath.toString());

        yantiandanganList.add(mYantianDangan);
        yantianDanganDao.add(mYantianDangan);
    }
