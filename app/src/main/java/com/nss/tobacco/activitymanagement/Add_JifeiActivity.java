...
@ContentView(R.layout.activity_add__jifei)
public class Add_JifeiActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    ImageView back_img;
    @ViewInject(R.id.second_tjsf_jifei_add_img)
    ImageView take_photo;
    @ViewInject(R.id.second_tjsf_jifei_add_gridView)
    GridView gridView;


    @ViewInject(R.id.second_tjsf_jifei_add_farmer)
    Spinner sp_farmer;

    @ViewInject(R.id.second_tjsf_jifei_add_time)
    TextView dateText1;

    @ViewInject(R.id.second_tjsf_jifei_add_type)
    Spinner sp_zhonglei;
    @ViewInject(R.id.second_tjsf_jifei_add_num)
    EditText shifeiliang;
    @ViewInject(R.id.second_tjsf_jifei_add_area)
    EditText mianji;
    @ViewInject(R.id.biaozhun_pic)
    ImageView biaozhun;
    @ViewInject(R.id.second_tjsf_jifei_add_detail)
    EditText edit_detail;
    @ViewInject(R.id.second_tjsf_jifei_add_btnSave)
    Button save;
    @ViewInject(R.id.second_tjsf_jifei_add_btnBack)
    Button back_btn;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        map = new HashMap<>();
        mContext = this;
        mDao = new Sec_JiFeiDao(mContext);
    ...
    private void initCareme() {
        ...
        if (getIntent().getStringExtra("folderName") == null) {
            photoFolderPath = CommonUtil.getSDPath() + File.separator + "JiFei";
        } else {
    ...
    private void setDate() {
        ...
        dateText1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                DateDialog dialog = new DateDialog(Add_JifeiActivity.this);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.back_item:
                ...
                break;
            //调用系统拍照
            case R.id.second_tjsf_jifei_add_img:
                //判断是否安装SD卡
                ...
                break;
            case R.id.second_tjsf_jifei_add_btnSave:
                //录入信息保存到数据库
                saveInfo();
                finish();
                break;
            case R.id.second_tjsf_jifei_add_btnBack:
                myDialog();
                break;
            default:
                break;
