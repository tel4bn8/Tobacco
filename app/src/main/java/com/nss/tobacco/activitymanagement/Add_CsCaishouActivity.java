...
@ContentView(R.layout.activity_add__cs_caishou)
public class Add_CsCaishouActivity extends AppCompatActivity implements View.OnClickListener{
    private Sec_CaishouDao caishouDao = null;
    ...
    @ViewInject(R.id.second_cscs_add_camera)
    private ImageView take_photo;
    @ViewInject(R.id.second_cscs_add_caremaView)
    private GridView caremaView;
    @ViewInject(R.id.second_cscs_add_spinner_zhongzhihu)
    private Spinner name;
    @ViewInject(R.id.second_cscs_add_text_start)
    private TextView dateText1;
    @ViewInject(R.id.second_cscs_add_text_end)
    private TextView dateText2;
    @ViewInject(R.id.second_cscs_add_edit_kangci)
    private EditText kangci;
    @ViewInject(R.id.second_cscs_add_edit_ganshu)
    private EditText ganshu;
    @ViewInject(R.id.second_cscs_add_spinner_anchengshudu)
    private Spinner anchengshu;
    @ViewInject(R.id.second_cscs_add_spinner_chengshudu)
    private Spinner chengshudu;
    @ViewInject(R.id.second_cscs_add_spinner_xianyanfenlei)
    private Spinner xianyanfenlei;
    @ViewInject(R.id.second_cscs_add_spinner_zhuanyedui)
    private Spinner zhuanyedui;
    @ViewInject(R.id.second_cscs_add_biaozhun_pic)
    private ImageView biaozhun;
    @ViewInject(R.id.second_cscs_add_edit_beizhu)
    private EditText beizhu;
    @ViewInject(R.id.second_cscs_add_btn_save)
    private Button btnsave;
    @ViewInject(R.id.second_cscs_add_btn_back)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        x.view().inject(this);
        context = this;
        caishouDao = new Sec_CaishouDao(this);
    ...
    private void setEditListener() {
        ...
        ganshu.addTextChangedListener(new TextWatcher() {
            @Override
            public void beforeTextChanged(CharSequence s, int start, int count, int after) {
                temp = s;
            }
            ...
            @Override
            public void afterTextChanged(Editable s) {
                editStart = ganshu.getSelectionStart();
                editEnd = ganshu.getSelectionEnd();
                if (temp.length() > 5) {//限制长度
                    Toast.makeText(getApplicationContext(),
                            "输入的字数已经超过了限制！", Toast.LENGTH_SHORT)
                            .show();
                    ...
                    ganshu.setText(s);
                    ganshu.setSelection(tempSelection2);
                }
            }
        });
        beizhu.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                if (temp.length() > 50) {//限制长度
                    ...
                    int tempSelection2 = editStart;
                    beizhu.setText(s);
                    beizhu.setSelection(tempSelection2);
                }
            }
        });
    }
    ...
    private void initCarema() {
        ...
        if(getIntent().getStringExtra("folderName") == null){
            photoFolderAddress = CommonUtil.getSDPath() + File.separator + "chengshucaishou";
        }else{
    ...
    private void setDate() {
        ...
        dateText1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                DateDialog dialog = new DateDialog(Add_CsCaishouActivity.this);
    ...
    private void mySpinner() {
        ...
        anchengshu.setAdapter(adapter2);
        ArrayAdapter<CharSequence> adapter3 = ArrayAdapter.createFromResource(this,R.array.shifou, R.layout.item_myspinner);
        chengshudu.setAdapter(adapter3);
        ArrayAdapter<CharSequence> adapter4 = ArrayAdapter.createFromResource(this,R.array.shifou, R.layout.item_myspinner);
        xianyanfenlei.setAdapter(adapter4);
        ArrayAdapter<CharSequence> adapter5 = ArrayAdapter.createFromResource(this,R.array.shifou, R.layout.item_myspinner);
        zhuanyedui.setAdapter(adapter5);
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.back_item:
                myDialog();
                break;
            case R.id.second_cscs_add_camera:
                ...
                break;
            case R.id.second_cscs_add_btn_save:
                saveInfo();
                Toast.makeText(this, "保存成功", Toast.LENGTH_SHORT).show();
                finish();
                break;
            case R.id.second_cscs_add_btn_back:
                myDialog();
                break;
            default:
                break;
    ...
    private void saveInfo() {
        ...
        ArrayList<Sec_CaishouEntity> caishouList = new ArrayList<>();
        Sec_CaishouEntity caishouEntity = new Sec_CaishouEntity();

        caishouEntity.setId(IdUtil.getId());
        caishouEntity.setFarmer(name.getSelectedItem().toString());
        caishouEntity.setStarttime(dateText1.getText().toString());
        caishouEntity.setEndtime(dateText2.getText().toString());
        caishouEntity.setPick(anchengshu.getSelectedItem().toString());
        caishouEntity.setSame(chengshudu.getSelectedItem().toString());
        caishouEntity.setKangci(kangci.getText().toString());
        caishouEntity.setGanci(ganshu.getText().toString());
        caishouEntity.setXianyanfenlei(xianyanfenlei.getSelectedItem().toString());
        caishouEntity.setZhuanyecaiji(zhuanyedui.getSelectedItem().toString());
        caishouEntity.setDetail(beizhu.getText().toString());
        caishouEntity.setState(state);
        caishouEntity.setPhotopath(photoPath.toString());

        caishouList.add(caishouEntity);
        caishouDao.add(caishouEntity);
    }
