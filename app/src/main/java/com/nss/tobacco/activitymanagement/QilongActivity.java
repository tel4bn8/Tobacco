...
@ContentView(R.layout.activity_qilong)
public class QilongActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    //查询到的所有数据
    private List<Sec_QiLongEntity> allEntites;
    private List<Sec_QiLongEntity> searchEntities;
    private List<Sec_QiLongEntity> stateEntities;
    private Sec_QiLongDao mDao = null;
    //全部数据或搜索 标志位
    ...
    private String photoFoloder= CommonUtil.getSDPath() + File.separator + "QiLong";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        x.view().inject(this);
        mDao = new Sec_QiLongDao(this);
    ...
    private void initData(List<Sec_QiLongEntity> entites) {
        mRecyclerView.setLayoutManager(new LinearLayoutManager(this));
        mRecyclerView.setAdapter(new CommonAdapter<Sec_QiLongEntity>(this, R.layout.item_detailmsg_recyclerview,entites) {
            @Override
            public void convert(ViewHolder holder, Sec_QiLongEntity entity) {
            ...
            public void onBindViewHolder(ViewHolder holder, final int position) {
                ...
                holder.setOnClickListener(R.id.id_ll_item,new View.OnClickListener() {
                    ...
                    public void onClick(View v){
                        ...
                        intent.setClass(QilongActivity.this, Sec_msg_QiLongActivity.class);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.back_item:
                ...
                break;
            case R.id.add_item:
                intent.setClass(this, Add_QilongActivity.class);
                startActivity(intent);
                break;
            //根据名字搜索
        ...
        if (!isSearch&&!isSelectState){
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    initData(allEntites);
                    int allCount=0;
                    for(Sec_QiLongEntity info:searchEntities){
                        if("1".equals(info.getState())){
                            allCount++;
                        }
                    }
                    if(allCount==searchEntities.size()){
                        Toast.makeText(QilongActivity.this, "上传成功", Toast.LENGTH_LONG).show();
                    }
                }
            }, 5000);
    ...
    //上传未上传的数据
    private void selectState(final List<Sec_QiLongEntity> entites) {
        ...
        for(int i=0;i<entites.size();i++){
            if("0".equals(entites.get(i).getState())){
                ...
                final int finalI = i;
                XUtil.Post(API_Data.QILONG,map, new MyCallBack<String>() {
                    @Override
                    public void onSuccess(String result) {
                        ...
                        if (!TextUtils.isEmpty(result)&&"1".equals(result)) {
                            ...
                            if(len>0) {
                                ...
                                Toast.makeText(QilongActivity.this, "上传失败,请检查网络", Toast.LENGTH_SHORT).show();
                            }
                        }
