...
@ContentView(R.layout.activity_dongguan)
public class DongguanActivity extends AppCompatActivity implements View.OnClickListener {
    //recycerView适配器
    ...
    //查询到的所有数据
    private List<Sec_DongGuanEntity> allEntites;
    private List<Sec_DongGuanEntity> searchEntities;
    private List<Sec_DongGuanEntity> stateEntities;
    private Sec_DongGuanDao mDao = null;
    //全部数据或搜索 标志位
    ...
    private String photoFoloder= CommonUtil.getSDPath() + File.separator + "DongGuan";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        x.view().inject(this);
        mDao = new Sec_DongGuanDao(this);
    ...
    private void initData(List<Sec_DongGuanEntity> entites) {
        mRecyclerView.setLayoutManager(new LinearLayoutManager(this));
        mRecyclerView.setAdapter(new CommonAdapter<Sec_DongGuanEntity>(this, R.layout.item_detailmsg_recyclerview,entites) {
            @Override
            public void convert(ViewHolder holder, Sec_DongGuanEntity entity) {
            ...
            public void onBindViewHolder(ViewHolder holder, final int position) {
                ...
                holder.setOnClickListener(R.id.id_ll_item,new View.OnClickListener() {
                    ...
                    public void onClick(View v){
                        ...
                        intent.setClass(DongguanActivity.this, Sec_msgDongGuanActivity.class);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.back_item:
                ...
                break;
            case R.id.add_item:
                intent.setClass(this, Add_DongguanActivity.class);
                startActivity(intent);
                break;
            //根据名字搜索
        ...
        if (!isSearch&&!isSelectState){
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    initData(allEntites);
                    int allCount=0;
                    for(Sec_DongGuanEntity info:searchEntities){
                        if("1".equals(info.getState())){
                            allCount++;
                        }
                    }
                    if(allCount==searchEntities.size()){
                        Toast.makeText(DongguanActivity.this, "上传成功", Toast.LENGTH_LONG).show();
                    }
                }
            }, 5000);
    ...
    //上传未上传的数据
    private void selectState(final List<Sec_DongGuanEntity> entites) {
        ...
        for(int i=0;i<entites.size();i++){
            if("0".equals(entites.get(i).getState())){
                ...
                final int finalI = i;
                XUtil.Post(API_Data.DONGGUAN,map, new MyCallBack<String>() {
                    @Override
                    public void onSuccess(String result) {
                        ...
                        if (!TextUtils.isEmpty(result)&&"1".equals(result)) {
                            ...
                            if(len>0) {
                                ...
                                Toast.makeText(DongguanActivity.this, "上传失败,请检查网络", Toast.LENGTH_SHORT).show();
                            }
                        }
                    }
