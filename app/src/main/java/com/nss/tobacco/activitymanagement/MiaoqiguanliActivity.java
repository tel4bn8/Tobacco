...
public class MiaoqiguanliActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    private Spinner year_spinner,state_spinner;//年度下拉选择框
    private EditText search_edit;//名字搜索框
    private Button search_btn;//搜索按钮
    private SwipeRefreshLayout mSwipeRefreshWidget;//SwipeRefreshLayout控件
    ...
    //查询到的所有数据
    private List<Sec_MiaoqiguanliEntity> allEntites;
    private List<Sec_MiaoqiguanliEntity> searchEntities;

    //采集状态里的数据
    private List<Sec_MiaoqiguanliEntity> stateEntities;
    ...
    private String photoFoloder= CommonUtil.getSDPath() + File.separator + "MiaoQiGuanLi";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_miaoqiguanli);

        mDao = new Sec_MiaoqiguanliDao(this);
    ...
    private void initView(){
        ...
        mSwipeRefreshWidget= (SwipeRefreshLayout) findViewById(R.id.second_ym_mq_swipeRefreshLayout);
        mRecyclerView = (RecyclerView) findViewById(R.id.second_ym_mq_recyclerView);
    ...
    private void initData(final List<Sec_MiaoqiguanliEntity> entites) {

        mRecyclerView.setAdapter(new CommonAdapter<Sec_MiaoqiguanliEntity>(this, R.layout.item_detailmsg_recyclerview,entites) {
            @Override
            public void convert(ViewHolder holder, Sec_MiaoqiguanliEntity entity) {
                holder.setText(R.id.id_num,entity.getCreatetime()+" ("+entity.getMiaoqi()+")");
            ...
            public void onBindViewHolder(ViewHolder holder, final int position) {
                ...
                holder.setOnClickListener(R.id.id_ll_item,new View.OnClickListener() {
                    ...
                    public void onClick(View v){
                        ...
                        intent.setClass(MiaoqiguanliActivity.this, Sec_msg_MiaoQiGuanLiActivity.class);
...
//添加点击事件
        ...
        switch (v.getId()) {
            case R.id.back_item:
                ...
                break;
            case R.id.add_item:
                intent.setClass(this, Add_MiaoqiActivity.class);
                startActivity(intent);
                break;
            //根据名字搜索
        ...
        if (!isSearch&&!isSelectState){
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    //allEntites上传的个数
                    int allCount=0;
                    for(Sec_MiaoqiguanliEntity info:allEntites){
                        if("1".equals(info.getState())){
                            allCount++;
                        }
                    }
                    if(allCount==allEntites.size()){
                        Toast.makeText(MiaoqiguanliActivity.this, "上传成功", Toast.LENGTH_LONG).show();
                    }
                }
            }, 5000);
        ...
        //搜索界面的数据
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    searchByName();
                    int searchCount=0;
                    for(Sec_MiaoqiguanliEntity info:searchEntities){
                        if("1".equals(info.getState())){
                            searchCount++;
                        }
                    }
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run(){
                    ...
                    initData(stateEntities);
                    int stateCount=0;
                    for(Sec_MiaoqiguanliEntity info:stateEntities){
                        if("1".equals(info.getState())){
                            stateCount++;
                        }
                    }
    ...
    //筛选未上传的数据,并上传数据
    private void selectState(final List<Sec_MiaoqiguanliEntity> entites) {
        ...
        for(int i=0;i<entites.size();i++){
            if("0".equals(entites.get(i).getState())){
                ...
                map.put("strong",entites.get(i).getZhuangmiaolv());
                map.put("detail",entites.get(i).getDetail());
                String miaoqi = entites.get(i).getMiaoqi();
                switch (miaoqi){
                    case "播种":
                        map.put("bozhongtime",entites.get(i).getCreatetime());
                        break;
                    case "出苗":
                        map.put("chumiaotime",entites.get(i).getCreatetime());
                        break;
                    case "间定苗":
                        map.put("jiandingmiaotime",entites.get(i).getCreatetime());
                        break;
                    case "剪叶":
                        map.put("jianyetime",entites.get(i).getCreatetime());
                        break;
                    case "炼苗":
                        map.put("lianmiaotime",entites.get(i).getCreatetime());
                        break;
                    case "供苗":
                        map.put("gongmiaotime",entites.get(i).getCreatetime());
                        break;
                }
                final int finalI = i;
                XUtil.Post(API_Data.MIAOQIGUANLI,map, new MyCallBack<String>() {
                    @Override
                    public void onSuccess(String result) {
                        ...
                        if(!TextUtils.isEmpty(result)&&"1".equals(result)){
                            ...
                            if(len>0) {
                                ...
                                Toast.makeText(MiaoqiguanliActivity.this, "上传失败,请检查网络", Toast.LENGTH_SHORT).show();
                            }
                        }else{
                            Toast.makeText(MiaoqiguanliActivity.this, "上传失败,请检查网络", Toast.LENGTH_SHORT).show();
                        }
