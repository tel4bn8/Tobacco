...
//揭膜中耕培土
@ContentView(R.layout.activity_add__jiemo)
public class Add_JiemoActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    @ViewInject(R.id.second_tjgl_jiemo_add_gridView)
    GridView gridView;

    @ViewInject(R.id.second_tjgl_jiemo_add_image)
    ImageView takePhoto;
    @ViewInject(R.id.second_tjgl_jiemo_add_farmer)
    Spinner spFarmer;
    @ViewInject(R.id.second_tjgl_jiemo_add_jiemo)
    Spinner spJiemo;

    @ViewInject(R.id.second_tjgl_jiemo_add_area)
    EditText etArea;

    @ViewInject(R.id.second_tjgl_jiemo_add_paishui)
    Spinner spPaiShui;

    @ViewInject(R.id.second_tjgl_jiemo_add_jiemotime)
    TextView tvJieMoTime;
    @ViewInject(R.id.second_tjgl_jiemo_add_xiaopeitu)
    TextView tvXiaoPeituTime;
    @ViewInject(R.id.second_tjgl_jiemo_add_dapeitu)
    TextView tvDaPeituTime;
    @ViewInject(R.id.second_tjgl_jiemo_add_height)
    EditText etHeight;
    @ViewInject(R.id.second_tjgl_jiemo_add_paishui)
    Spinner spKaiGou;


    @ViewInject(R.id.second_tjgl_jiemo_add_detail)
    EditText etDetail;
    @ViewInject(R.id.second_tjgl_jiemo_add_btnSave)
    Button btnSave;
    @ViewInject(R.id.second_tjgl_jiemo_add_btnBack)
    Button btnBack;
    @ViewInject(R.id.biaozhun_pic)
    ImageView biaozhun;

    Context mContext;
    private Sec_JiemoDao jieMoDao = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        map = new HashMap<>();
        mContext = this;
        jieMoDao = new Sec_JiemoDao(mContext);
    ...
    private void initCareme(){
        ...
        if (getIntent().getStringExtra("folderName") == null){
            photoFolderPath = CommonUtil.getSDPath() + File.separator + "JieMo";
        } else {
            photoFolderPath = getIntent().getStringExtra("folderName");
        }
    }
    private void initView() {
        tvTitle.setText("揭膜中耕培土");
    }
    ...
    private void setDate() {
        //时间选择
        tvJieMoTime.setText(GetDate.lastDay());
        tvJieMoTime.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View arg0) {
                DateDialog dialog = new DateDialog(Add_JiemoActivity.this);
                dialog.setDate(tvJieMoTime);
            }
        });
        tvXiaoPeituTime.setText(GetDate.lastDay());
        tvXiaoPeituTime.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                DateDialog dialog = new DateDialog(Add_JiemoActivity.this);
                dialog.setDate(tvXiaoPeituTime);
            }
        });

        tvDaPeituTime.setText(GetDate.lastDay());
        tvDaPeituTime.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                DateDialog dialog = new DateDialog(Add_JiemoActivity.this);
                dialog.setDate(tvDaPeituTime);
            }
        });
    }
    //自定义spinner
    private void mySpinner() {
        ...
        ArrayAdapter<CharSequence> adapter2 = ArrayAdapter.createFromResource(this,R.array.kaigou, R.layout.item_myspinner);
        spKaiGou.setAdapter(adapter2);
        ArrayAdapter<CharSequence> adapter3 = ArrayAdapter.createFromResource(this,R.array.kaigou, R.layout.item_myspinner);
        spJiemo.setAdapter(adapter3);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.back_item:
                ...
                break;
            //拍照片 事件
            case R.id.second_tjgl_jiemo_add_image:
               //判断是否安装SD卡
                ...
                break;
            //保存信息
            case R.id.second_tjgl_jiemo_add_btnSave:
                //录入信息保存到数据库
                saveInfo();
                finish();
                break;
            //返回信息
            case R.id.second_tjgl_jiemo_add_btnBack:
    ...
    private void saveInfo() {
        ...
        info.setJiemo(spJiemo.getSelectedItem().toString());
        info.setJiemotime(tvJieMoTime.getText().toString());
        info.setXiaopeitu(tvXiaoPeituTime.getText().toString());
        info.setDapeitu(tvDaPeituTime.getText().toString());
        info.setHeight(etHeight.getText().toString());
        info.setArea(etArea.getText().toString());
        info.setPaishui(spPaiShui.getSelectedItem().toString());
        info.setDetail(etDetail.getText().toString());
