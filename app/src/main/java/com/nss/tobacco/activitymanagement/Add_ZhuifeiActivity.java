...
@ContentView(R.layout.activity_add__zhuifei)
public class Add_ZhuifeiActivity extends AppCompatActivity implements View.OnClickListener{
    @ViewInject(R.id.textView)
    TextView textView;
    @ViewInject(R.id.back_item)
    ImageView back_img;

    @ViewInject(R.id.second_tjsf_zhuifei_add_img)
    ImageView take_photo;
    @ViewInject(R.id.second_tjsf_zhuifei_add_gridView)
    GridView gridView;

    @ViewInject(R.id.second_tjsf_zhuifei_add_farmer)
    Spinner sp_farmer;

    @ViewInject(R.id.second_tjsf_zhuifei_add_time)
    TextView dateText1;
    @ViewInject(R.id.second_tjsf_zhuifei_add_type)
    Spinner sp_zhonglei;

    @ViewInject(R.id.second_tjsf_zhuifei_add_num)
    EditText shifeiliang;

    @ViewInject(R.id.second_tjsf_zhuifei_add_area)
    EditText mianji;

    @ViewInject(R.id.biaozhun_pic)
    ImageView biaozhun;

    @ViewInject(R.id.second_tjsf_zhuifei_add_detail)
    EditText edit_detail;
    @ViewInject(R.id.second_tjsf_zhuifei_add_btnSave)
    Button save;
    @ViewInject(R.id.second_tjsf_zhuifei_add_btnBack)
    Button back_btn;
    ...
    private CaremaAdapter mCaremaAdapter = null;//存放图片的GridView的适配器
        ...
        protected void onCreate(Bundle savedInstanceState) {
            ...
            map = new HashMap<>();
            mContext = this;
            mDao = new Sec_ZhuifeiDao(mContext);
    ...
    private void initCareme() {
        ...
        if (getIntent().getStringExtra("folderName") == null) {
            photoFolderPath = CommonUtil.getSDPath() + File.separator + "ZhuiFei";
        } else {
    ...
    private void setDate() {
        ...
        dateText1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                DateDialog dialog = new DateDialog(Add_ZhuifeiActivity.this);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.back_item:
                myDialog();
                break;
            case R.id.second_tjsf_zhuifei_add_img:
                //判断是否安装SD卡
                ...
                break;
            case R.id.second_tjsf_zhuifei_add_btnSave:
                //录入信息保存到数据库
                saveInfo();
                finish();
                break;
            case R.id.second_tjsf_zhuifei_add_btnBack:
                myDialog();
                break;
            default:
                break;
    ...
    private void saveInfo() {
        ...
        Sec_ZhuifeiEntity info = new Sec_ZhuifeiEntity();
