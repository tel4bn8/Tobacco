...
@ContentView(R.layout.activity_add__wenshidu)
public class Add_WenshiduActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    Context mContext;
    private Sec_WenshiduDao wenshiduDao = null;
    ...
    @ViewInject(R.id.second_ym_wsdgl_addPhotoImage)
    ImageView take_photo;
    @ViewInject(R.id.second_ym_wsdgl_gridView)
    GridView photoGridView;

    @ViewInject(R.id.second_ym_wsdgl_farmer)
    Spinner farmer;
    @ViewInject(R.id.second_ym_wsdgl_wendu)
    EditText wendu;
    @ViewInject(R.id.second_ym_wsdgl_shidu)
    EditText shidu;

    @ViewInject(R.id.second_ym_wsdgl_mioaqing)
    Spinner spMiaoqing;

    TextView dateText1;//创建时间

    @ViewInject(R.id.biaozhun_pic)
    ImageView biaozhun;

    @ViewInject(R.id.second_ym_wsdgl_detail)
    EditText beizhu;

    @ViewInject(R.id.second_ym_wsdgl_btnSave)
    Button btnSave;
    @ViewInject(R.id.second_ym_wsdgl_btnBack)
    Button backBtn;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        map = new HashMap<>();
        mContext = this;
        wenshiduDao = new Sec_WenshiduDao(mContext);
    ...
    private void initCareme() {
        ...
        if (getIntent().getStringExtra("folderName") == null) {
            photoFolderPath = CommonUtil.getSDPath() + File.separator + "WenShiDuGuanLi";
        } else {
    ...
    private void setDate() {
        //时间选择
        dateText1 = (TextView) findViewById(R.id.second_ym_wsdgl_createTime);
        dateText1.setText(GetDate.lastDay());
        dateText1.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View arg0) {
                DateDialog dialog = new DateDialog(Add_WenshiduActivity.this);
    ...
    private void mySpinner() {
        ...
        spMiaoqing.setAdapter(adapter2);
    }
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.back_item:
                ...
                break;
            //调用系统拍照
            case R.id.second_ym_wsdgl_addPhotoImage:
                //判断是否安装SD卡
                if (CommonUtil.getSDPath() == null) {
                    Toast.makeText(Add_WenshiduActivity.this, "请安装SD卡", Toast.LENGTH_SHORT).show();
                    return;
                }
                //判断图片个数是否超过最大
                if (mListPhotos != null && mListPhotos.size() >= Constants.MAX_PHOTO_SIZE) {
                    Toast.makeText(Add_WenshiduActivity.this, "最多允许拍摄" + Constants.MAX_PHOTO_SIZE, Toast.LENGTH_SHORT).show();
                    return;
                }
                //调用系统拍照
                ...
                break;
            case R.id.second_ym_wsdgl_btnSave:
                //录入信息保存到数据库
                saveInfo();
                finish();
                break;
            case R.id.second_ym_wsdgl_btnBack:
                myDialog();
                break;
            default:
                break;
    ...
    private void saveInfo() {
        ...
        info.setWendu(wendu.getText().toString());
        info.setShidu(shidu.getText().toString());
        ...
        wenshiduDao.add(info);
        Toast.makeText(Add_WenshiduActivity.this, "保存成功", Toast.LENGTH_SHORT).show();
    }
    //返回按钮提醒框
