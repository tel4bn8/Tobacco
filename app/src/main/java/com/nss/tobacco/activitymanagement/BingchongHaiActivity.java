...
public class BingchongHaiActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private Sec_BingchongFzDao mDao = null;
    //查询到的所有数据
    private List<Sec_BingchongFzEntity> allEntites;
    //搜索条件的数据
    private List<Sec_BingchongFzEntity> searchEntities;
    //采集状态里的数据
    private List<Sec_BingchongFzEntity> stateEntities;
    ...
    private String photoFoloder = CommonUtil.getSDPath() + File.separator + "BingChongHaiFangZhi";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_bingchong);
        mDao = new Sec_BingchongFzDao(this);
    ...
    private void initView() {
        ...
        mRecyclerView = (RecyclerView) findViewById(R.id.second_ym_bcfz_recyclerview);
        mRecyclerView.setLayoutManager(new LinearLayoutManager(this));
        //RecyclerView分割线
        mRecyclerView.addItemDecoration(new DividerItemDecoration(this, DividerItemDecoration.VERTICAL_LIST));

        mSwipRefreshLayout = (SwipeRefreshLayout) findViewById(R.id.second_ym_bcfz_swipeRefreshWidget);
        //初始化下拉刷新控件的颜色改变
    ...
    private void initData(List<Sec_BingchongFzEntity> entites) {
        mRecyclerView.setLayoutManager(new LinearLayoutManager(this));
        mRecyclerView.setAdapter(new CommonAdapter<Sec_BingchongFzEntity>(this, R.layout.item_detailmsg_recyclerview, entites) {
            @Override
            public void convert(ViewHolder holder, Sec_BingchongFzEntity entity) {
            ...
            public void onBindViewHolder(ViewHolder holder, final int position) {
                ...
                holder.setOnClickListener(R.id.id_ll_item, new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        intent.setClass(BingchongHaiActivity.this, Sec_msg_BingChongHaiFzActivity.class);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()){
            case R.id.back_item:
                ...
                break;
            case R.id.add_item:
                intent.setClass(this, Add_BingchongFangzhiActivity.class);
                startActivity(intent);
                break;
            //根据名字搜索
            case R.id.up_item:
        ...
        if (!isSearch&&!isSelectState){
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    initData(allEntites);
                    int allCount=0;
                    for(Sec_BingchongFzEntity info:searchEntities){
                        if("1".equals(info.getState())){
                            allCount++;
                        }
                    }
                    if(allCount==searchEntities.size()){
                        Toast.makeText(BingchongHaiActivity.this, "上传成功", Toast.LENGTH_LONG).show();
                    }
                }
            }, 6000);
    ...
    //上传未上传的数据
    private void selectState(final List<Sec_BingchongFzEntity> entites) {
        ...
        for (int i = 0; i < entites.size(); i++) {
            if ("0".equals(entites.get(i).getState())) {
                ...
                map.put("rate", entites.get(i).getRate());
                map.put("area", entites.get(i).getArea());
                map.put("loss", entites.get(i).getLoss());
                map.put("createtime", entites.get(i).getCreatetime());
                map.put("detail", entites.get(i).getDetail());
                final int finalI = i;
                XUtil.Post(API_Data.BINGCHONGHAI, map, new MyCallBack<String>() {
                    @Override
                    public void onSuccess(String result) {
                        ...
                        if (!TextUtils.isEmpty(result)&&"1".equals(result)) {
                            ...
                            if(len>0) {
                                ...
                                Toast.makeText(BingchongHaiActivity.this, "上传失败,请检查网络", Toast.LENGTH_SHORT).show();
                            }
                        }
