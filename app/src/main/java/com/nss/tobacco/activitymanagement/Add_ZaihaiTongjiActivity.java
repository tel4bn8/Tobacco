...
@ContentView(R.layout.activity_add__zaihai_tongji)
public class Add_ZaihaiTongjiActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    @ViewInject(R.id.second_zhtj_add_photo)
    ImageView take_photo;
    @ViewInject(R.id.second_zhtj_add_caremaView)
    private GridView caremaView;

    @ViewInject(R.id.second_zhtj_add_spinner_zhongzhihu)
    private Spinner name;
    @ViewInject(R.id.second_zhtj_add_mianji_edit)
    private EditText mianji;
    @ViewInject(R.id.second_zhtj_add_hetongmianji_edit)
    private EditText hetongmianji;
    @ViewInject(R.id.second_zhtj_add_sunshi_edit)
    private EditText sunshi;
    @ViewInject(R.id.second_zhtj_add_spinner_shouzaizhonglei)
    private Spinner zhonglei;
    @ViewInject(R.id.second_zhtj_add_shijian_text)
    private TextView dateText1;
    @ViewInject(R.id.second_zhtj_add_beizhu_edit)
    private EditText beizhu;
    @ViewInject(R.id.second_zhtj_add_btn_save)
    private Button save;
    @ViewInject(R.id.second_zhtj_add_btn_back)
    ...
    private void setEditListener() {
        mianji.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                if (temp.length() > 5) {//限制长度
                    ...
                    mianji.setText(s);
                    mianji.setSelection(tempSelection1);
                }
            }
        });
        ...
        sunshi.addTextChangedListener(new TextWatcher() {
            @Override
            public void beforeTextChanged(CharSequence s, int start, int count, int after) {
                temp = s;
            }
            ...
            @Override
            public void afterTextChanged(Editable s) {
                editStart = sunshi.getSelectionStart();
                editEnd = sunshi.getSelectionEnd();
                if (temp.length() > 6) {//限制长度
                    ...
                    sunshi.setText(s);
                    sunshi.setSelection(tempSelection3);
                }
            }
        });
    ...
    private void initCarema() {
        ...
        if(getIntent().getStringExtra("folderName") == null){
            photoFolderAddress = CommonUtil.getSDPath() + File.separator + "zaihaitongji";
        }else{
    ...
    private void setDate() {
        ...
        dateText1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                DateDialog dialog = new DateDialog(Add_ZaihaiTongjiActivity.this);
    ...
    private void mySpinner() {
        ...
        ArrayAdapter<CharSequence> adapter2 = ArrayAdapter.createFromResource(this,R.array.zaihai, R.layout.item_myspinner);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.back_item:
                myDialog();
                break;
            case R.id.second_zhtj_add_photo:
                //验证sd卡是否可用
                ...
                break;
            case R.id.second_zhtj_add_btn_save:
                saveInfo();
                Toast.makeText(this, "保存成功", Toast.LENGTH_SHORT).show();
                finish();
                break;
            case R.id.second_zhtj_add_btn_back:
                myDialog();
                break;
            default:
                break;
    ...
    private void saveInfo() {
        ...
        ArrayList<Sec_Zaihaitongji> zaihaiList = new ArrayList<>();
        Sec_Zaihaitongji mSec_Zaihaitongji = new Sec_Zaihaitongji();

        mSec_Zaihaitongji.setId(IdUtil.getId());
        mSec_Zaihaitongji.setFarmer(name.getSelectedItem().toString());
        mSec_Zaihaitongji.setType(zhonglei.getSelectedItem().toString());
        mSec_Zaihaitongji.setArea1(mianji.getText().toString());
        mSec_Zaihaitongji.setArea2(hetongmianji.getText().toString());
        mSec_Zaihaitongji.setLose(sunshi.getText().toString());
        mSec_Zaihaitongji.setCreatetime(dateText1.getText().toString());
        mSec_Zaihaitongji.setDetail(beizhu.getText().toString());
        mSec_Zaihaitongji.setState(state);
        mSec_Zaihaitongji.setPhotopath(photoPath.toString());

        zaihaiList.add(mSec_Zaihaitongji);
        sec_zaihaitongjiDao.add(mSec_Zaihaitongji);
    }
