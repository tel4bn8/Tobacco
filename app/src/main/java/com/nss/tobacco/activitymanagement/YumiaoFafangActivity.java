...
public class YumiaoFafangActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    private Sec_YumiaofafangDao mDao = null;
    //查询到的所有数据
    private List<Sec_YumiaoFafangEntity> allEntites;
    private List<Sec_YumiaoFafangEntity> searchEntities;
    private List<Sec_YumiaoFafangEntity> stateEntities;
    //全部数据或搜索 标志位
    ...
    private String photoFoloder= CommonUtil.getSDPath() + File.separator + "YuMiaoFaFang";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_yumiaofafang);
        initView();
        mDao = new Sec_YumiaofafangDao(this);
    ...
    private void initView() {
        ...
        mSwipRefresh= (SwipeRefreshLayout) findViewById(R.id.second_ym_ymff_swipeRefresh);
        //初始化下拉刷新控件的颜色改变
    ...
    private void initData(List<Sec_YumiaoFafangEntity> entites) {
        mRecyclerView.setLayoutManager(new LinearLayoutManager(this));
        mRecyclerView.setAdapter(new CommonAdapter<Sec_YumiaoFafangEntity>(this, R.layout.item_detailmsg_recyclerview,entites) {
            @Override
            public void convert(ViewHolder holder, Sec_YumiaoFafangEntity entity) {
            ...
            public void onBindViewHolder(ViewHolder holder, final int position) {
                ...
                holder.setOnClickListener(R.id.id_ll_item,new View.OnClickListener() {
                    ...
                    public void onClick(View v){
                        ...
                        intent.setClass(YumiaoFafangActivity.this, Sec_msg_YuMiaoFaFangActivity.class);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.back_item:
                ...
                break;
            case R.id.add_item:
                intent.setClass(this, Add_YumiaoFafangActivity.class);
                startActivity(intent);
                break;
            //根据名字搜索
        ...
        if (!isSearch&&!isSelectState){
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    initData(allEntites);
                    int allCount=0;
                    for(Sec_YumiaoFafangEntity info:searchEntities){
                        if("1".equals(info.getState())){
                            allCount++;
                        }
                    }
                    if(allCount==searchEntities.size()){
                        Toast.makeText(YumiaoFafangActivity.this, "上传成功", Toast.LENGTH_LONG).show();
                    }
                }
            }, 5000);
    ...
    //筛选未上传的数据,并上传数据
    private void selectState(final List<Sec_YumiaoFafangEntity> entites) {
        ...
        for(int i=0;i<entites.size();i++){
            if("0".equals(entites.get(i).getState())){
                ...
                final int finalI = i;
                XUtil.Post(API_Data.YUMIAOFANGFA,map, new MyCallBack<String>() {
                    @Override
                    public void onSuccess(String result) {
                        ...
                        if (!TextUtils.isEmpty(result)&&"1".equals(result)) {
                            ...
                            if(len>0) {
                                ...
                                Toast.makeText(YumiaoFafangActivity.this, "上传失败,请检查网络", Toast.LENGTH_SHORT).show();
                            }
                        }
