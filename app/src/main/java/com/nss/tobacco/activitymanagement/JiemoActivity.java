...
import com.nss.tobacco.daos.Sec_ZhuifeiDao;
...
public class JiemoActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    private Sec_JiemoDao mDao = null;
    //查询到的所有数据
    private List<Sec_JiemoEntity> allEntites;
    private List<Sec_JiemoEntity> searchEntities;
    private List<Sec_JiemoEntity> stateEntities;
    ...
    private String photoFoloder= CommonUtil.getSDPath() + File.separator + "JieMo";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_jiemo);
        mDao = new Sec_JiemoDao(this);
    ...
    private void initView() {
        ...
        mRecyclerView = (RecyclerView) findViewById(R.id.second_tjgl_jiemo_recyclerView);
        mSwipRefresh = (SwipeRefreshLayout) findViewById(R.id.second_tjgl_jiemo_swipeRefreshWidget);
        textView.setText("揭膜中耕培土");
        //初始化下拉刷新控件的颜色改变
    ...
    private void initData(List<Sec_JiemoEntity> entites) {
        mRecyclerView.setLayoutManager(new LinearLayoutManager(this));
        mRecyclerView.setAdapter(new CommonAdapter<Sec_JiemoEntity>(this, R.layout.item_detailmsg_recyclerview,entites) {
            @Override
            public void convert(ViewHolder holder, Sec_JiemoEntity entity) {

                holder.setText(R.id.id_num,entity.getJiemotime());
            ...
            public void onBindViewHolder(ViewHolder holder, final int position) {
                ...
                holder.setOnClickListener(R.id.id_ll_item,new View.OnClickListener() {
                    ...
                    public void onClick(View v){
                        ...
                        intent.setClass(JiemoActivity.this, Sec_msg_JieMoActivity.class);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.back_item:
                ...
                break;
            case R.id.add_item:
                intent.setClass(this, Add_JiemoActivity.class);
                startActivity(intent);
                break;
            //根据名字搜索
        ...
        if (!isSearch&&!isSelectState){
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    initData(allEntites);
                    int allCount=0;
                    for(Sec_JiemoEntity info:searchEntities){
                        if("1".equals(info.getState())){
                            allCount++;
                        }
                    }
                    if(allCount==searchEntities.size()){
                        Toast.makeText(JiemoActivity.this, "上传成功", Toast.LENGTH_LONG).show();
                    }
                }
            }, 5000);
    ...
    //上传未上传的数据
    private void selectState(final List<Sec_JiemoEntity> entites) {
        ...
        for(int i=0;i<entites.size();i++){
            if("0".equals(entites.get(i).getState())){
                ...
                map.put("water",entites.get(i).getPaishui());
                map.put("mo",entites.get(i).getJiemo());
                map.put("motime",entites.get(i).getJiemotime());
                map.put("smalltime",entites.get(i).getXiaopeitu());
                map.put("bigtime",entites.get(i).getDapeitu());
                map.put("high",entites.get(i).getHeight());
                map.put("area",entites.get(i).getArea());
                map.put("pianshu",entites.get(i).getPianshu());
                map.put("detail",entites.get(i).getDetail());
                final int finalI = i;
                XUtil.Post(API_Data.JIEMO,map, new MyCallBack<String>() {
                    @Override
                    public void onSuccess(String result) {
                        ...
                        if (!TextUtils.isEmpty(result)&&"1".equals(result)) {
                            ...
                            if(len>0) {
                                ...
                                Toast.makeText(JiemoActivity.this, "上传失败,请检查网络", Toast.LENGTH_SHORT).show();
                            }
                        }
