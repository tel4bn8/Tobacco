...
@ContentView(R.layout.activity_zhuifei)
public class ZhuifeiActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    @ViewInject(R.id.second_tjsf_zhuifei_add_swipRefresh)
    SwipeRefreshLayout mSwipeRefreshWidget;
    @ViewInject(R.id.second_tjsf_zhuifei_add_recycler)
    RecyclerView mRecyclerView;

    private Sec_ZhuifeiDao mDao = null;

    //查询到的所有数据
    private List<Sec_ZhuifeiEntity> allEntites;
    private List<Sec_ZhuifeiEntity> searchEntities;
    private List<Sec_ZhuifeiEntity> stateEntities;
    //全部数据或搜索 标志位
    ...
    private String photoFoloder= CommonUtil.getSDPath() + File.separator + "ZhuiFei";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        x.view().inject(this);
        mDao = new Sec_ZhuifeiDao(this);
    ...
    private void initData(List<Sec_ZhuifeiEntity> entites) {
        mRecyclerView.setAdapter(new CommonAdapter<Sec_ZhuifeiEntity>(this, R.layout.item_detailmsg_recyclerview,entites) {
            @Override
            public void convert(ViewHolder holder, Sec_ZhuifeiEntity entity) {
            ...
            public void onBindViewHolder(ViewHolder holder, final int position) {
                ...
                holder.setOnClickListener(R.id.id_ll_item,new View.OnClickListener() {
                    ...
                    public void onClick(View v){
                        ...
                        intent.setClass(ZhuifeiActivity.this, Sec_msgZhuiFeiActivity.class);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.back_item:
                ...
                break;
            case R.id.add_item:
                intent.setClass(this, Add_ZhuifeiActivity.class);
                startActivity(intent);
                break;
            //根据名字搜索
        ...
        if (!isSearch&&!isSelectState){
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    initData(allEntites);
                    int allCount=0;
                    for(Sec_ZhuifeiEntity info:searchEntities){
                        if("1".equals(info.getState())){
                            allCount++;
                        }
                    }
                    if(allCount==searchEntities.size()){
                        Toast.makeText(ZhuifeiActivity.this, "上传成功", Toast.LENGTH_LONG).show();
                    }
                }
            }, 5000);
    ...
    //上传未上传的数据
    private void selectState(final List<Sec_ZhuifeiEntity> entites) {
        for(int i=0;i<entites.size();i++){
            if("0".equals(entites.get(i).getState())){
                ...
                final int finalI = i;
                XUtil.Post(API_Data.ZHUIFEI,map, new MyCallBack<String>() {
                    @Override
                    public void onSuccess(String result) {
                        ...
                        if (!TextUtils.isEmpty(result)&&"1".equals(result)) {
                            ...
                            if(len>0) {
                                ...
                                Toast.makeText(ZhuifeiActivity.this, "上传失败,请检查网络", Toast.LENGTH_SHORT).show();
                            }
                        }
