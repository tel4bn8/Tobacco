...
public class HongkaoActivity extends AppCompatActivity implements View.OnClickListener {
    private Sec_HongkaoDao sec_hongkaoDao = null;

    private List<Sec_HongkaoEntity> allentity; //所有数据
    private List<Sec_HongkaoEntity> searchentity;//查询结果数据
    ...
    private String photoFoloder = CommonUtil.getSDPath() + File.separator + "hongkaoguanli";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_hongkao);
        sec_hongkaoDao = new Sec_HongkaoDao(this);
        allentity = sec_hongkaoDao.searchAllData();
    ...
    protected void initData(List<Sec_HongkaoEntity> entites) {
        recyclerView.setAdapter(new CommonAdapter<Sec_HongkaoEntity>(this, R.layout.item_detailmsg_recyclerview,entites) {
            @Override
            public void convert(ViewHolder holder, Sec_HongkaoEntity mSec_HongkaoEntity) {
                holder.setText(R.id.id_num,mSec_HongkaoEntity.getCreatetime());
                holder.setText(R.id.id_farmer,mSec_HongkaoEntity.getFarmer());

                if("0".equals(mSec_HongkaoEntity.getState())){
            ...
            public void onBindViewHolder(ViewHolder holder, final int position) {
                ...
                holder.setOnClickListener(R.id.id_ll_item,new View.OnClickListener() {
                    ...
                    public void onClick(View v){
                        ...
                        intent.setClass(HongkaoActivity.this, Sec_msg_HongkaoActivity.class);

                        if(!isSearch){
                            intent.putExtra("hongkaoguanliInfo", allentity.get(position));
                        }else{
                            intent.putExtra("hongkaoguanliInfo", searchentity.get(position));
                        }
    ...
    private void initView() {
        ...
        recyclerView = (RecyclerView) findViewById(R.id.second_hk_recyclerview);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));

        mSwipeRefreshLayout = (SwipeRefreshLayout) findViewById(R.id.second_hk_swipe);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.back_item:
                ...
                break;
            case R.id.add_item:
                intent.setClass(this, Add_HongkaoActivity.class);
                startActivity(intent);
                break;
            case R.id.up_item:
        ...
        if(!isSearch){
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    allentity=sec_hongkaoDao.searchAllData();
                    initData(allentity);
                    Toast.makeText(HongkaoActivity.this, "上传成功", Toast.LENGTH_SHORT).show();
                }
            }, 1000);
        }
        //搜索界面的数据
    ...
    private void searchInfoByName() {
        ...
        isSearch=true;
        searchentity = sec_hongkaoDao.searchData(village);
    ...
    //上传未上传的数据
    private void upData(final List<Sec_HongkaoEntity> entites) {
        for(int i=0;i<entites.size();i++){
            if("0".equals(entites.get(i).getState())){
                ...
                map.put("beforeweight",entites.get(i).getBeforeweight());
                map.put("afterweight",entites.get(i).getAfterweight());
                map.put("electric",entites.get(i).getElectric());
                map.put("coal",entites.get(i).getCoal());
                map.put("zhuanyehongkao",entites.get(i).getZhuanyehongkao());
                map.put("quqingquza",entites.get(i).getQuqingquza());
                map.put("money",entites.get(i).getMoney());
                map.put("createtime",entites.get(i).getCreatetime());
                map.put("detail",entites.get(i).getDetail());
                final int finalI = i;
                XUtil.Post(API_Data.HongkaoUP,map, new MyCallBack<String>() {
                    @Override
                    public void onSuccess(String result) {
                        super.onSuccess(result);
                        if(!TextUtils.isEmpty(result)){
                            //修改该条目数据库中的状态
                            int len = sec_hongkaoDao.updateStateByFarmer("1", entites.get(finalI).getId());
                            if(len>0){
