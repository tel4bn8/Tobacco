...
@ContentView(R.layout.activity_donggeng)
public class DonggengActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    //查询到的所有数据
    private List<Sec_DongGengEntity> allEntites;
    private List<Sec_DongGengEntity> searchEntities;

    private List<Sec_DongGengEntity> stateEntities;
    private Sec_DongGengDao mDao = null;
    //全部数据或搜索 标志位
    ...
    private String photoFoloder= CommonUtil.getSDPath() + File.separator + "DongGeng";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        x.view().inject(this);
        mDao = new Sec_DongGengDao(this);
    ...
    private void initData(List<Sec_DongGengEntity> entites) {
        mRecyclerView.setAdapter(new CommonAdapter<Sec_DongGengEntity>(this, R.layout.item_detailmsg_recyclerview,entites) {
            @Override
            public void convert(ViewHolder holder, Sec_DongGengEntity entity) {
            ...
            public void onBindViewHolder(ViewHolder holder, final int position) {
                ...
                holder.setOnClickListener(R.id.id_ll_item,new View.OnClickListener() {
                    ...
                    public void onClick(View v){
                        ...
                        intent.setClass(DonggengActivity.this, Sec_msg_DengGengActivity.class);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.back_item:
                ...
                break;
            case R.id.add_item:
                intent.setClass(this, Add_DonggengActivity.class);
                startActivity(intent);
                break;
            //根据名字搜索
        ...
        if (!isSearch&&!isSelectState){
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    initData(allEntites);
                    int allCount=0;
                    for(Sec_DongGengEntity info:searchEntities){
                        if("1".equals(info.getState())){
                            allCount++;
                        }
                    }
                    if(allCount==searchEntities.size()){
                        Toast.makeText(DonggengActivity.this, "上传成功", Toast.LENGTH_LONG).show();
                    }
                }
            }, 6000);
    ...
    //上传未上传的数据
    private void selectState(final List<Sec_DongGengEntity> entites) {
        ...
        for(int i=0;i<entites.size();i++){
            if("0".equals(entites.get(i).getState())){
                ...
                map.put("depth",entites.get(i).getDepth());
                map.put("area",entites.get(i).getArea());
                map.put("detail",entites.get(i).getDetail());
                final int finalI = i;
                XUtil.Post(API_Data.DONGGENG,map, new MyCallBack<String>() {
                    @Override
                    public void onSuccess(String result) {
                        ...
                        if (!TextUtils.isEmpty(result)&&"1".equals(result)) {
                            ...
                            if(len>0) {
                                ...
                                Toast.makeText(DonggengActivity.this, "上传失败,请检查网络", Toast.LENGTH_SHORT).show();
                            }
                        }
